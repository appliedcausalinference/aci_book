...
Judging by trends in machine learning research, it becomes clear that interest in causal methods is growing beyond traditional settings in applied statistics as well. Figure \@ref(fig:neurips-causal) shows the increase in causal papers that appear at the popular ML research journal NeurIPS^[Source: https://richardjoncarter.com/research/research-into-causal-ai-has-grown-significantly-in-the-last-3-years-if-neurips-is-anything-to-go-by/].

```{r neurips-causal, fig.cap="Causal papers featured at NeurIPS", fig.align='center', echo=FALSE, out.width='75%'}
knitr::include_graphics("img/causalpapersNeurIPS.png")
```
...
To illustrate this, we'll use the standard dataset investigating gender bias in admissions at UC Berkeley in 1973^[Accessed via https://discovery.cs.illinois.edu/dataset/berkeley/]. Figure \@ref(fig:simpson1) shows the high-level admission rate for male and female applicants, where there appears to be a significant bias against female students.

```{r simpson1, echo=FALSE, fig.cap="Plot of high-level admission rates by gender", fig.align='center', message=FALSE}
admissions <- read.csv('https://waf.cs.illinois.edu/discovery/berkeley.csv') %>%
  filter(Major != 'Other') %>% 
  mutate(admission_int = case_when(Admission == 'Accepted' ~ 1, TRUE ~ 0),
         female_int = case_when(Gender == 'F' ~ 1, TRUE ~ 0))

ggplot(admissions, aes(x = Gender, y = admission_int, fill=Gender)) + 
  geom_bar(stat='summary', fun='mean', alpha=0.8, width=0.75) + 
  scale_fill_brewer(palette = 'Set1') + 
  scale_y_continuous(labels = scales::percent) + 
  theme_bw() + 
  labs(title="Possible Gender Bias in College Admissions", y="Admission Rate", x="Gender") + 
  guides(fill="none")
```

As we can see, the overall admission rate of male applicants is _much_ higher than for female applicants: a difference of roughly 30% versus 50%! This appears very concerning, however, things become less clear when looking at admission rate within each major separately. Figure \@ref(fig:simpson2) shows the male and female admission rates for each major. Surprisingly, the trend actually reverses for most of the majors in the dataset.

```{r simpson2, echo=FALSE, fig.cap="Reversal of trend when controlling for major", fig.align='center', message=FALSE}
admissions_grouped <- admissions %>% 
  group_by(Gender, Major) %>% 
  summarise(admission_rate = mean(admission_int))

ggplot(admissions_grouped, aes(x = Major, y = admission_rate, fill=Gender)) + 
  geom_bar(stat='identity', position='dodge', alpha=0.8) + 
  scale_fill_brewer(palette = 'Set1') + 
  scale_y_continuous(labels = scales::percent) + 
  theme_bw() + 
  labs(title="Admission Rate by Gender and Major", y="Admission Rate", x="Major") 
```

_But how can this be?_ If female admission rates are equal or higher within most departments, shouldn't the overall female admission rate also be higher? This is a perfect example of Simpson's Paradox, and it has to do with unequal distribution of applicants between departments. Figure \@ref(fig:simpson3) shows one aspect contributing to this phenomenon: the departments with the highest acceptance rates are overwhelmingly male-dominated. 

```{r simpson3, fig.cap='Admission rates and gender distribution by major', fig.align='center', echo=FALSE}
p1 <- ggplot(admissions, aes(x = Major, y = admission_int)) + 
  geom_bar(stat='summary', fun='mean', alpha=0.8, width=0.8) + 
  scale_y_continuous(labels = scales::percent) + 
  theme_bw() + 
  labs(title="Overall Admission Rate by Major", y=NULL, x="Major")

p2 <- ggplot(admissions, aes(x = Major, y = female_int)) + 
  geom_bar(stat='summary', fun='mean', alpha=0.8, width=0.8) + 
  scale_y_continuous(labels = scales::percent) + 
  theme_bw() + 
  labs(title="Percentage of Applicants Who Are Female", y=NULL, x="Major")
...
```{r gender-regression, echo=F}
summary(glm(admission_int ~ Gender + Major, data=admissions))
```
...
|[CausalML](https://github.com/uber/causalml) | `CausalML` is a Python library designed to easily calculate treatment effects from observational data, including uplift modeling and many of the meta-learners introduced in \@ref(estimationprocess) |
