...
### Probabilistic Kripke Structures {#PKS}
...
In \@ref(populationMeasures), we learned about the average treatment effect as a way of quantifying the relationship between a treatment (which is a potential cause) and and outcome (an observed effect). [@Kleinberg2010TheTL] introduces a quantity called the average causal significance (ACS), $\epsilon_{avg}$, that quantifies the causal significance of a potential cause $c$ to an effect $e$, where $c \leadsto {}^{\geq r, \leq s}_{\geq p} e$; i.e. after $c$ becomes true, $e$ occurs in a time window $[r, s]$, with probability $p$:
