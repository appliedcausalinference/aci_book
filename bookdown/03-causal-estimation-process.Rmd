# Causal Inference: A Practical Approach {#causalestimation}
...
In the last chapter, we introduced the causality flowchart for estimating the causal effect, as shown in Fig. \@ref(fig:causalestimandtoestimate1).

```{r causalestimandtoestimate1, fig.cap='Causality flowchart: From a target causal estimand to an estimate', out.width="90%", echo=FALSE, fig.align='center'}
knitr::include_graphics('img/PotentialOutcomeEstimation.png')
```

The two pertinent questions will be answered in this chapter, as shown in Fig. \@ref(fig:causalestimandtoestimate2).
...
```{r causalestimandtoestimate2, fig.cap='Causality Flowchart: How to go from a target causal estimand to an estimate using causal modeling', out.width="90%", echo=FALSE, fig.align='center'}
knitr::include_graphics('img/PotentialOutcomeEstimationHow.png')
```

>Causal models can be constructed either by experts with domain knowledge or through automated procedures known as causal discovery (discussed in the next chapter). Building causal models requires domain knowledge, identifying relevant variables, and determining the relationships between them.
...
Notably, unlike the machine learning process, the causal inference process involves distinct steps, as Dow et al. underscored in their work [@dowhypaper] and given by Fig. \@ref(fig:inferencingprocess).


```{r inferencingprocess, fig.cap='Causal inference process in practice', out.width="90%", echo=FALSE, fig.align='center'}
...
>A cause in a directed edged graph (DAG), similar to Bayesian networks, is defined as if any changes are made to a node, a response, or corresponding modifications are observed in the connected node(s). In the context of graphical models, a causal graph is a type of Bayesian network in which each node's direct causes are represented by its parents.

For a comprehensive and in-depth exploration of causal graphs, we highly recommend referring to Judea Pearl's book, "Causal Inference in Statistics."

### Assumptions in Causal Modeling

Fig.\@ref(fig:causalgraph) shows the entire flow of assumptions that help in causal modeling.
...
```{r causalgraph, fig.cap='Causal Graph Assumptions', echo=FALSE,fig.align='center'}
...
A chain is a sequence of nodes such that each node is a parent of the next node in the sequence as shown in Fig.\@ref(fig:chains).
...
```{r chains, fig.show = "hold", out.width='40%',fig.cap='Association flow in immoralities without conditioning and with conditioning', echo=FALSE,fig.align='center'}
knitr::include_graphics('img/chain-flow.png')
knitr::include_graphics('img/chain-blocked.png')
```

#### Forks

In a fork, two variables have a single parent between them, as shown in Fig\@ref(fig:forks). Similar to chains, there is dependence between $X_1$ and $X_2$ and between $X_2$ and $X_3$ because of the causal edges assumption. $X_1$ and $X_3$ are dependent as there is a flow of association or statistical dependence from $X_1$ to $X_3$ through $X_2$. Also, if we condition on the $X_2$, $X_1$ and $X_3$ are independent. In other words, $X_2$ will block the association flow between $X_1$ and $X_3$ by conditioning.

```{r forks, fig.show = "hold", out.width='40%',fig.cap='Association flow in forks without and with conditioning', echo=FALSE,fig.align='center'}
knitr::include_graphics('img/fork-flow.png')
knitr::include_graphics('img/fork-blocked.png')
```

#### Immoralities

Immoralities refer to a configuration in a directed acyclic graph where a single node relies on two parents who are not directly connected, as depicted in Fig.\@ref(fig:immorality). The node $X_2$ in this structure is called a _collider_ and obstructs the flow of association between nodes $X_1$ and $X_3$ without conditioning, unlike in chains or forks. However, unlike chains or forks, by conditioning on the collider $X_2$, the flow of association or dependence persists between $X_1$ and $X_3$.

```{r immorality, fig.show = "hold", out.width='40%',fig.cap='Association flow in forks without and with conditioning', echo=FALSE,fig.align='center'}
knitr::include_graphics('img/immorality.png')
knitr::include_graphics('img/immorality-cond.png')
```
...
1. If there exists a node $W$ on the path from $X$ to $Y$ such that it is part of a chain structure ($X$ -> $W$ -> $Y$) or a fork structure ($X$ <- $W$ -> $Y$), and $W$ is conditioned on ($W \in Z$).
...
$$ X \ind_G Y |Z \implies X \ind_P Y|Z $$
...
```{r conditionalvsinterventional, fig.cap='Conditional vs Interventiona', out.width="80%", echo=FALSE, fig.align='center'}
...
It states that if a node $X_i$ is being intervened, then only the mechanism $P(x_i|pa_i)$ changes and the intervention has no impact on any other mechanism changes, i.e., $P(x_j|pa_j)$ where $i \neq j$ remain unchanged. Modularity assumptions is graphically demonstrated in Fig \@ref(fig:parents). The violation of the modularity assumption implies that when a node is intervened, mechanisms of other non-parent nodes changes, and thus any local effect assumption goes away for computation purposes.

```{r parents, fig.cap='Modularity and Interventions', out.width="80%", echo=FALSE, fig.align='center'}
knitr::include_graphics('img/Parents.png')
```

### Modularity Assumptions and Truncated Factorization

The network factorization for Fig.\@ref(fig:parents) can be written as :
...
The truncated factorization can be employed to estimate the causal effect of treatment $T$ on outcome $Y$ in a simple graph comprising three variables $X$, $Y$, and $T$, with $X$ as the confounder as shown in Fig.\@ref(fig:truncated). The aim is to estimate the causal quantity $P(y|do(T))$.


```{r truncated, fig.cap='Modularity and Interventions', out.width="50%", echo=FALSE, fig.align='center'}
...
$$ Y := f(X, U)$$
When there are many cause-effect relationships, as shown in Fig. \@ref(fig:scm), the representation using structural causal models equation with exogenous, endogenous, and noise variables is given by:
$$X = f_X(W,U_X)$$
$$Z = f_Z(X,Y,U_Z)$$



```{r scm, fig.cap='Structural Causal Models', out.width="70%", echo=FALSE, fig.align='center'}
...
For the basic causal model as shown in Fig. \@ref(fig:scmintervention), with single confounder $X$ affecting both the treatment $T$ and the outcome $Y$, the SCM equations can  be written as:
$$ T = f_T(X, U_T)$$
$$Y = f_Y(X,T, U_Y)$$
...
```{r scmintervention, fig.cap='Intervention and removing the edge corresponding to $do(T=t)$', out.width="50%", echo=FALSE, fig.align='center'}
...
>The identification process helps in converting the causal estimands to statistical estimands. i.e., to go from $P(Y|do(t))$ to $P(Y|t)$. If we can go from $P(Y|do(t))$ to $P(Y|t)$, we have __identifiability__.

Let us consider a simple identification process with one treatment ($T$), one confounding variable ($X$), and one outcome ($Y$) as given in Fig. \@ref(fig:identification)

```{r identification, fig.cap='Identification', out.width="50%", echo=FALSE, fig.align='center'}
...
>As highlighted in the overall process illustrated in Fig. \@ref(fig:identificationmath), the identification process involves a transformation from causal estimand $P(y|do(T))$ to statistical estimand ($\mathbb{E}_{X} P(y|t,X)$ or $P(y|t)$) contingent upon the existence or absence of confounding variables.

```{r identificationmath, fig.cap='Identification in presence or absence of confouding variable ', out.width="90%", echo=FALSE, fig.align='center'}
...
Judea Pearl justifies the use of the front-door adjustment method through the illustration of an example in which the effect of smoking (treatment) on cancer (outcome) is studied while taking into account the influence of tar (observed mediator) and an unknown genotype (unobserved confounder), as depicted in Fig. \@ref(fig:pearlfrontdoor) [@pearl2010causal]. In such Directed Acyclic Graphs (DAGs), the backdoor criterion is inapplicable due to an unobserved confounder.

```{r pearlfrontdoor, fig.cap='Cancer and Smoking relationship', out.width="50%", echo=FALSE, fig.align='center'}
knitr::include_graphics('img/PearlFrontdoor.png')
```

The more generic DAG is shown in Fig. \@ref(fig:genericfrontdoor) The intuition behind the front-door adjustment can be broken into the following three steps as follows:

```{r genericfrontdoor, fig.cap='Generalized Front-door', out.width="50%", echo=FALSE, fig.align='center'}
...
In situations where specific variables affect the treatment variable(s) but do not directly influence the outcome variable, identification can be achieved through instrumental variable analysis. For instance, consider the example of three variables, namely smoking, cigarette prices, and cancer, as shown in Fig.\@ref(fig:ivexample). It is apparent that cigarette prices affect whether an individual smokes but do not directly impact the likelihood of developing cancer. Such variables that affect the treatment variable(s) but not the outcome variable are referred to as instrumental variables, as described by Pearl [@pearl2010causal].

```{r ivexample, fig.cap='Example of Instrumental Variables', out.width="50%", echo=FALSE, fig.align='center'}
...
Thus, when generalized, as shown in Fig.\@ref(fig:ivgeneral), the identification process is a two-stage process. The first step is to measure the effect of the instrument variable on the treatment using regression as given by:
...
```{r ivgeneral, fig.cap='Generalized relationship between Instrumental Variables, Treatment, and Outcome', out.width="50%", echo=FALSE, fig.align='center'}
...
Fig.\@ref(fig:regressiondiscontinuity) shows an example of student GPA as the outcome ($Y$) on the y-axis, and student test scores normalized as a variable ($X$) on the x-axis with a threshold deciding scholarship ($T$) and its impact as a shifted GPA on the y-axis. The shift is the regression discontinuity.
...
```{r regressiondiscontinuity, fig.cap='Regression Discontinuity', out.width="60%", echo=FALSE, fig.align='center'}
...
```{r did, fig.cap='Difference-in-Differences', out.width="50%", echo=FALSE, fig.align='center'}
knitr::include_graphics('img/DID.png')
```


To make things concrete, let us consider a simple use case of a binary treatment ($T=0, T=1$), with a real-valued outcome ($y \in \mathbb R$), and the outcome is measured w.r.t to time as shown in Fig. \@ref(fig:did) Regression analysis can be done using a couple of variables: $D$ for treatment and $T$ for time as given by:
...
$$P(y|do(x),z,w) = P(y|do(x),w)\ if\ (Y \ind Z|X,W)_{G_{\overline{X}}}$$
...
$$P(y|do(x),do(z),w) = P(y|do(x),z,w)\ if\ (Y \ind Z|X,W)_{G_{{\overline{X}}{\underline{Z}}}}$$
...
$$P(y|do(x),do(z),w) = P(y|do(x),w)\ if\ (Y \ind Z|X,W)_{G_{\overline{XZ(W)}}}$$
...
#### Conditional Outcome Modeling Estimator (COM Estimator or S-Learner) {#slearner}
...
### TARNet
COM estimators combine treatment $T$ and the input covariates $W$, making the estimator biased towards zero. The GCOM estimator builds two separate estimators for each treatment (binary $T=0$ and $T=1$), and since it does not use all the data, it leads to a higher variance model. TARNet estimators can combine the two by first learning a representation $\hat{\mu}$ from all the input covariates, and then the layer can branch to two heads as shown in Fig. \@ref(fig:tarnet)[@shalit2017estimating].


```{r tarnet, fig.cap='TARNet estimator', out.width="50%", echo=FALSE, fig.align='center'}
...
Fig.\@ref(fig:matching) shows a simplified view with two dimensions $(X_1,X_2)$ how the nearest neighbor (1-NN) can be used for matching between the treated and the control group.

```{r matching, fig.cap='Matching Algorithm', out.width="50%", echo=FALSE, fig.align='center'}
...
Double machine learning estimators, as the name suggests, use machine learning to learn estimators in two stages to "partial out" the confounders (and other covariates)  $X$ as shown in Fig. \@ref(fig:doubleml) [@chernozhukov2018double].

```{r doubleml, fig.cap='Double Machine Learning', out.width="50%", echo=FALSE, fig.align='center'}
...
In the simplest case, we assume an unobserved confounder $U$ along with an observed confounder $W$ as shown in Fig. \@ref(fig:observedconfounding). Thus, the ATE can be written using the adjustment formula as:

```{r observedconfounding, fig.cap='Observed Confounding', out.width="50%", echo=FALSE, fig.align='center'}
...
Considering a simple setting with observed variable $W$ and an unobserved variable $U$ with only linear impacts as shown in Fig.\@ref(fig:sensitivityanalysis), the structural causal model equations as a linear function will be:

```{r sensitivityanalysis, fig.cap='Sensitivity Analysis', out.width="50%", echo=FALSE, fig.align='center'}
...
A contour plot for different values of $\beta_u$ and $\alpha_u$ gives the sensitivity to single unobserved confounding as shown in Fig. \@ref(fig:sensitivityplot)

```{r sensitivityplot, fig.cap='Sensitivity Plot', out.width="50%", echo=FALSE, fig.align='center'}
...
```
The causal graph showing the relationships between the outcome, treatment, and observed confounders is shown in Fig.\@ref(fig:causallalonde)

```{r causallalonde, fig.cap='Causal Graph for Identification', out.width="50%", echo=FALSE, fig.align='center'}
