...
```{r causalexample, fig.cap='An Illustrative Example', out.width="50%", echo=FALSE, fig.align='center'}
knitr::include_graphics('img/CausalIllustrativeExample.png')
```
In the context of causal inference, consider an example involving a cancer patient as shown in \@ref(fig:causalexample). The patient's diagnosis determines the treatment plan, and both variables affect the outcome of survival time. The diagnosis is represented by the variable $X$, which could be health records, medical images, etc. The Treatment $T$, which is binary in this example (e.g., surgery or chemotherapy), but could be continuous in other cases, affects the survival time, which is a continuous variable represented by the outcome $Y$. It is assumed that the treatment is determined once based on the diagnosis and does not change. The treated outcome $Y_i(1)$ represents the outcome for surgery, while the control outcome $Y_i(0)$ represents the outcome for non-surgery (chemotherapy).
...
## Population Level Treatment Effects {#populationMeasures}
...
>Thus, ATE summarizes the treatment effect that applies to the entire target population. ATT is used when the interest lies in estimating the average causal effect of a treatment on those who received the treatment. CATE aims to understand the heterogeneous treatment effects and identify factors that moderate the impact of the treatment.
...
> Ignorability assumption enables us to compute the ATE by ignoring the unknowns because $\mathbb{E}[Y(T=1)|T=1]- \mathbb{E}[Y(T=0)|T=0] = \mathbb{E}[Y|T=1]- \mathbb{E}[Y|T=0]$
...
Thus the entire logical flow from a target causal estimand to a corresponding estimate is shown in Fig. \@ref(fig:causalestimandtoestimate).

```{r causalestimandtoestimate, fig.cap='Causality Flowchart: From a Target Causal Estimand to an Estimate', out.width="90%", echo=FALSE, fig.align='center'}
...
The histogram plot of the post-test scores distributed across different grades, as presented in Fig.\@ref(fig:pergradedist), illustrates a considerable degree of variation among the test scores across various grades.

```{r pergradedist, fig.cap='Scores Per Grade Distribution for Treated and Control ', echo=FALSE,fig.align='center'}
...
The figure Fig.\@ref(fig:regressiontreatment5095) displays general effectiveness, particularly in the lower grades, albeit the large standard errors of estimation make it challenging to ascertain with certainty.

```{r regressiontreatment5095, fig.cap='Regression on treatment indicator with 50 and 95 percent uncertainty intervals', echo=FALSE,fig.align='center'}
...
Incorporating the interaction of treatment with pre-test introduces additional complexity. The estimated treatment effect, expressed as $17 - 0.15x$, cannot be readily interpreted without knowledge of the corresponding range of $x$. It is worth noting that pre-test scores have been observed to range between approximately $80$ and $120$ from the grade $4$ plot as in Fig.\@ref(fig:regressiontreatmentpretestandinteraction). Within this range, the estimated treatment effect varies between $17-0.15·80=5$  for classes with pre-test scores of $80$, and $17-0.15·120=-1$ for classes with pre-test scores of $120$. The observed range illustrates the variation in estimated treatment effects with respect to pre-test scores, and does not represent the uncertainty associated with the estimation of treatment effects. To quantify the uncertainty, a graphical representation of the estimated treatment effects as a function of $x$ can be generated, superimposing random simulation draws on the plot as shown in Fig.\@ref(fig:uncertaintyplot).

The estimated treatment effect as a function of pre-test score is denoted by the dark line in Fig.\@ref(fig:uncertaintyplot). This line represents the difference between the two regression lines in the grade 4 plot, as depicted in Fig.\@ref(fig:regressiontreatmentpretestandinteraction). We can compute the treatment effect and is $1.8$ with a standard error of $0.7$ – similar to the result from the model adjusting for pre-test but with no interactions.



```{r uncertaintyplot, fig.cap='Treatment effect in grade with 50 and 90 perent uncertainty intervals for Grade 4 with interactions', echo=FALSE,fig.align='center'}
...
```{r withsupplement, fig.cap='Estimated effect of supplement compared to replacement with 50 and 95 percent uncertainty intervals', echo=FALSE,fig.align='center'}
knitr::include_graphics('img/SupplementEffect.png')
```

The level of uncertainty is substantial, rendering the comparison inconclusive, barring grade $2$ where the conclusion is less ambiguous as shown in Fig.\@ref(fig:withsupplement). Nevertheless, overall the results align with the reasonable supposition that supplementing is more efficacious than replacing in the lower grades.
